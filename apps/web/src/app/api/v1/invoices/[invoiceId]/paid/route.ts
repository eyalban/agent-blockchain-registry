 ...
 * Body: { txHash } — the tx that settled the invoice.
 */
export async function POST(
  ...
  try {
    ...
    await markInvoicePaid({
      ...
      paidBlock: verified.blockNumber,
      paidTxHash: parsed.data.txHash,
    })

    return NextResponse.json({ invoiceId, status: 'paid' })
  } catch (err) {
    if (err instanceof EventVerificationError) {
      const { status, body } = errorToResponse(err)
      return NextResponse.json(body, { status })
    }
    console.error('invoice paid POST', err)
    return NextResponse.json(
